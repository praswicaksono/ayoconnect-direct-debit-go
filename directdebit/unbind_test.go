...
var _ = Describe("Unbind", func() {
	var (
		...
		request    *directdebit.AccountUnbindRequest
		b2bToken   string
		b2b2cToken string
		externalID string
	)
	...
	When("debit error", func() {
		It("return error when failed to execute http", func() {
			...
			resp, err := client.Unbind(context.Background(), request, b2bToken, b2b2cToken, externalID)
	...
	When("debit successful", func() {
		It("return correct response", func() {
			expectedResp := &directdebit.AccountUnbindResponse{
			...
			Expect(resp.ReferenceNo).Should(Equal("t4tn57kibeunbam9dtr89urv8h2jbem9"))
		})
	})
})
